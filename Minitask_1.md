...
Using the Raspberry Pi Pico SDK's TinyUSB library as its USB software stack, and it's built-in USB Audio Class support transforms it into a USB microphone device.
The RP2040 does not have built-in support for both PDM peripheral interface in software. So, output clock signal is generated to the the PDM MEMS Microphone on one pin and another pin receives data from the microphone.
